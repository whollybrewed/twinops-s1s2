...
oms.newResources("s1s2.root:root.ssv")
...
for message in consumer:
    ...
    try: 
        if message.topic == 'inkbird2' and message.offset == inkbirdOffsetNow:
            ...
            oms.setReal('s1s2.Root.Temperature.T_amb', tempInkbird)
            logger.info("Get T_amb (Temperature): %s", Types.Status(status))
            oms.setReal('s1s2.Root.HeatTransfer.Tamb', tempInkbird)
        ...
        if message.topic == 'tilt2' and message.offset == tiltOffsetNow:
            ...
            oms.setReal('s1s2.Root.Temperature.T_0', tempTilt)
            oms.setReal('s1s2.Root.control_model.T_sensor', tempTilt)
            logger.info("Get T_0: %s", Types.Status(status))
            oms.setReal('s1s2.Root.Temperature.sg', sgTilt)
            logger.info("Get SG: %s", Types.Status(status))
    except:
        pass

status = oms.export(model, "s1s2.ssp")
logger.info("Update SSP: %s", Types.Status(status))

repo = Repo(".")
logger.info("Local repository: %s", repo)
adds = repo.index.add(['s1s2.ssp'])
